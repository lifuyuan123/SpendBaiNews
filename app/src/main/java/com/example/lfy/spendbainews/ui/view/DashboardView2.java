...
public class DashboardView2 extends View {
    ...
    private void init() {
        ...
        mTexts = new String[]{"0", "较差", "200", "中等", "400", "良好", "600", "优秀", "800", "极好", "1000+"};
        mBgColors = new int[]{ContextCompat.getColor(getContext(), R.color.transparent),
...
//        canvas.drawText(mHeaderText, mCenterX, mCenterY - dp2px(30), mPaint);
...
//        mPaint.setAlpha(160);
//        mPaint.setTextSize(sp2px(10));
//        canvas.drawText(getFormatTimeStr(), mCenterX, mCenterY+dp2px(60), mPaint);
    }
    ...
    private float calculateRelativeAngleWithValue(int value) {
        ...
        if (value > 800) {
            ...
            return 8 * degreePerSection + 2 * degreePerSection / 200 * (value - 800);
        } else if (value > 600) {
            return 6 * degreePerSection + 2 * degreePerSection / 200 * (value - 600);
        } else if (value > 400) {
            return 4 * degreePerSection + 2 * degreePerSection / 200 * (value - 400);
        } else if (value > 200) {
            return 2 * degreePerSection + 2 * degreePerSection / 200 * (value - 200);
        } else {
            return 2 * degreePerSection / 200 * (value);
        }
    }
    ...
    private String calculateCreditDescription() {
        if (mSolidCreditValue > 800) {
            return "信用极好";
        } else if (mSolidCreditValue > 600) {
            return "信用优秀";
        } else if (mSolidCreditValue > 400) {
            return "信用良好";
     ...
     * @param creditValue 信用值
     */
    public void setCreditValueWithAnim(final int creditValue) {
        if (creditValue < mMin || !isAnimFinish) {
            return;
        }

        ValueAnimator creditValueAnimator;
        if (creditValue>mMax){
            creditValueAnimator = ValueAnimator.ofInt(0, mMax);
        }else {
            creditValueAnimator = ValueAnimator.ofInt(0, creditValue);
        }
        ...
        creditValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    mSolidCreditValue=mCreditValue;
                    postInvalidate();
                    Log.e("onAnimationUpdate",mCreditValue+"mCreditValue");
            }
        });

        // 计算最终值对应的角度，以扫过的角度的线性变化来播放动画
        float degree = calculateRelativeAngleWithValue(creditValue);
...
//                mSolidCreditValue= (int) Math.floor(((Float) animation.getAnimatedValue()));
//                Log.e("onAnimationUpdate",mSolidCreditValue+"");
            }
        });
